...
public class CommentJsonTest {
    ...
    @Autowired
    private JacksonTester<CommentInfoDto> testerInfoDto;
    ...
    void commentInfoDtoSerialize() throws IOException {
        ...
        JsonContent<CommentInfoDto> json = testerInfoDto.write(dto);
    ...
    void commentDtoInfoDeserialize() throws IOException {
        CommentInfoDto dto = CommentMapper.toCommentInfoDto(COMMENT);
        String content = gson.toJson(dto);
        CommentInfoDto result = testerInfoDto.parse(content).getObject();
